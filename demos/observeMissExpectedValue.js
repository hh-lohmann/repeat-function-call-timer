...
/** Observe process that misses expected value
 *  - ! Demo to *miss* an expected value
 *    - cf. observeMatchExpectedValue.js = Demo to *match* an expected value
 *  - For demonstrating functionality, not for direct application
 * @example observeMissExpectedValue(document.getElementById('myOutput'))
 ...
 */
export const observeMissExpectedValue = function(targetEl,onEnd=()=>{},duration=3000,stepLength=100){
  ...
  const observedProcess = function(){
    setTimeout(()=>observedProcessFulfilled=true,4200);
  }
